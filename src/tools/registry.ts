/**
 * Tool registry for Liberia Law MCP Server.
...
import { getLiberianImplementations, type GetLiberianImplementationsInput } from './get-liberian-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including Liberia Law (National Council for Law Reporting). ' +
    ...
    'Call this FIRST when you need to understand what Liberian legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Liberian statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (article) from an Liberian proclamation or regulation. ' +
  ...
  },
  {
    ...
    description:
      'Validate an Liberian legal citation against the database — zero-hallucination check. ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Liberian statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Liberia?" ' +
  ...
  },
  {
    ...
    description:
      'Format a Liberian legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether an Liberian proclamation or provision is currently in force, amended, or repealed. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get the EU/international legal basis that a Liberian statute aligns with or references. ' +
      'Liberia is not an EU member but several Liberian laws were influenced by EU regulations ' +
    ...
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Liberian statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_liberian_implementations',
    description:
      'Find all Liberian statutes that align with or implement a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Liberian statutes. ' +
      'Note: Liberia implements international standards through domestic legislation influenced by international frameworks.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that have Liberian implementing/aligning legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_liberian_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents with Liberian aligning legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU/international legal basis for a SPECIFIC provision within a Liberian statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU/international alignment status for a Liberian statute or provision. ' +
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_liberian_implementations':
          result = await getLiberianImplementations(db, args as unknown as GetLiberianImplementationsInput);
          break;
