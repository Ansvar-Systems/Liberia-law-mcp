/**
 * search_eu_implementations — Search EU directives/regulations that have Liberian implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_liberian_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as liberian_statute_count
  ...
  if (input.has_liberian_implementation) {
    sql += ' HAVING liberian_statute_count > 0';
  }
