/**
 * get_liberian_implementations — Find Liberian statutes implementing a specific EU directive/regulation
...
export interface GetLiberianImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface LiberianImplementationResult {
  document_id: string;
...
export async function getLiberianImplementations(
  db: InstanceType<typeof Database>,
  input: GetLiberianImplementationsInput,
): Promise<ToolResponse<LiberianImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as LiberianImplementationResult[];
