#!/usr/bin/env tsx
/**
 * Liberia Law MCP -- Census-Driven Ingestion Pipeline (PDF-based)
 *
 * Reads data/census.json and fetches + parses every ingestable Act
 * from lawliberian.com. Liberian proclamations are published as PDFs.
 *
 * Pipeline per act:
 ...
 *
 * Data source: lawliberian.com (Federal Negarit Gazette)
...
async function downloadPdf(url: string, outputPath: string): Promise<{ status: number; size: number }> {
  ...
  const response = await fetch(url, {
    headers: {
      'User-Agent': 'liberian-law-mcp/1.0 (https://github.com/Ansvar-Systems/liberian-law-mcp; hello@ansvar.ai)',
    },
    redirect: 'follow',
  });
...
async function main(): Promise<void> {
  ...
  console.log('Liberia Law MCP -- Ingestion Pipeline (PDF-based)');
  console.log('===================================================\n');
  console.log(`  Source: lawliberian.com (Federal Negarit Gazette)`);
  ...
  console.log(`\n  Source:      lawliberian.com (Federal Negarit Gazette PDFs)`);
