#!/usr/bin/env tsx
/**
 * Liberian Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'http://lawliberian.com';
const PORTAL_NAME = 'Liberia Law';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/liberian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Liberian Law MCP — Data Freshness Check');
