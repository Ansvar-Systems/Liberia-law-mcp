#!/usr/bin/env tsx
/**
 * Database builder for Liberia Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Liberia Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Liberian statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('source', 'lawliberian.com');
